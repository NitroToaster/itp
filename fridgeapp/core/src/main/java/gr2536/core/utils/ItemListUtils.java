...
public class ItemListUtils {
    ...
    public static int clampAdd(int a, int b) {
    ...
    public static boolean nameMatches(String name, String query, NameMatchMode mode) {
     ...
     */
    public static boolean nameMatchesAll(String name, String query) {
        if (query == null) return true;
        String n = name == null ? "" : name.trim().toLowerCase(Locale.ROOT);
        String q = query.trim().toLowerCase(Locale.ROOT);
        if (q.isEmpty()) return true;
